...
subroutine xc_get_vxc(der, xcs, st, rho, ispin, ioniz_pot, qtot, vxc, ex, ec, deltaxc, vtau)
  ...
  integer :: i,j, k, n1, n2, n3, idx, curr_idx, iter, flatten_idx, iix, iiy, iiz, iip
  ...
  print*, "Parallel check ", xcs%parallel
  ...
  print*, "Density l_dens shape check ", shape(l_dens)
  print*, "Density l_dens max min", maxval(l_dens), minval(l_dens)
  ...
  print*, "Exchange energy test! ", ex
  print*, "Correlation energy test! ", ec
  ...
  !print*, "Fc1_W"
  !do i=1,1000
  !	write(*,2)  (fc1_W(i,j),j=1,2)
  !end do
  !print*, "Fc2_W"
  !do i=1,500
  ! 	write(*,1001)  (fc2_W(i,j),j=1,1001)
  !end do
  !print*, "Fcout_W"
  !do i=1,1
  ! 	write(*,501)  (fcout_W(i,j),j=1,501)
  !end do
  
  print*, "Weights check fc1_W", maxval(fc1_W), minval(fc1_W), sum(fc1_W)/float(2000)
  print*, "Weights check fc2_W", maxval(fc2_W), minval(fc2_W), sum(fc2_W)/float(500500)
  print*, "Weights check fcout_W", maxval(fcout_W), minval(fcout_W), sum(fcout_W)/float(501)
  
  print*, "Density dens shape check ", shape(dens)
  !print*, "Density l_dens shape check ", shape(l_dens)
  !print*, "Density l_dens max min", maxval(l_dens), minval(l_dens)
  ...
  print*, "Test preprocess_input", test_out_preprocess(1,1), test_out_preprocess(2,1), test_out_preprocess(3,1), test_out_preprocess(4,1)
  ...
  if(order_file_exists) then
	...
	do ip = 1, der%mesh%np
		...
		!print*, "iix iiy iiz iip", iix, iiy, iiz, iip
  ...
  !rho_3d = reshape( rho, (/ n1, n2, n3 /), order = order1)
  ...
  !	do j = 1, n2
  ! 		do k = 1, n3
  !			print*, "rho_3d(",i,",",j,",",k,") = ", rho_3d(i,j,k)
  ! 		end do
  !  end do
  !end do
  ...
  print*, "Der mesh np global", der%mesh%np_global
  
  print*, "Flags gga mgga mgga_withexc, libvdwxc check", gga, mgga, mgga_withexc, libvdwxc
  ...
  if (gga) then
	  do ip = 1, der%mesh%np
		...
		do iter = 1, 5
			if (curr_idx == -1) then
				neigh_indices_i(iter) = n1 - 1
			else if (curr_idx == 0) then
				neigh_indices_i(iter) = n1
			else if (curr_idx == (n1 + 1)) then
				neigh_indices_i(iter) = 1
			else if (curr_idx == (n1 + 2)) then
				neigh_indices_i(iter) = 2
			else
				neigh_indices_i(iter) = curr_idx
			end if
			curr_idx = curr_idx + 1
		end do
		curr_idx = j - 2
		do iter = 1, 5
			if (curr_idx == -1) then
				neigh_indices_j(iter) = n2 - 1
			else if (curr_idx == 0) then
				neigh_indices_j(iter) = n2
			else if (curr_idx == (n2 + 1)) then
				neigh_indices_j(iter) = 1
			else if (curr_idx == (n2 + 2)) then
				neigh_indices_j(iter) = 2
			else
				neigh_indices_j(iter) = curr_idx
			end if
			curr_idx = curr_idx + 1
		end do
		curr_idx = k - 2
		do iter = 1, 5
			if (curr_idx == -1) then
				neigh_indices_k(iter) = n3 - 1
			else if (curr_idx == 0) then
				neigh_indices_k(iter) = n3
			else if (curr_idx == (n3 + 1)) then
				neigh_indices_k(iter) = 1
			else if (curr_idx == (n3 + 2)) then
				neigh_indices_k(iter) = 2
			else
				neigh_indices_k(iter) = curr_idx
			end if
			curr_idx = curr_idx + 1
		end do
		!print*, "Checking calculation , i j k ", i , j, k
		!print*, "Checking calculation, neigh indices i ", neigh_indices_i
		!print*, "Checking calculation, neigh indices j ", neigh_indices_j
		!print*, "Checking calculation, neigh indices k ", neigh_indices_k
		iter = 1
		do i = 1, 5
			do j = 1, 5
				do k = 1, 5
					flatten_idx = (neigh_indices_i(i) - 1)*n2*n3 + (neigh_indices_j(j) - 1)*n3 + (neigh_indices_k(k) - 1) + 1
					!vxc_inp_matrix_gga(iter, 1) = rho_3d(neigh_indices_i(i), neigh_indices_j(j), neigh_indices_k(k))
					if(order_file_exists) then
						if(corrected_rho(flatten_idx, 1) <= 0) then 
							vxc_inp_matrix_gga(iter, 1) = -27
						else
							vxc_inp_matrix_gga(iter, 1) = log10(corrected_rho(flatten_idx, 1))
						end if
					else
						if(rho(flatten_idx, 1) <= 0) then 
							vxc_inp_matrix_gga(iter, 1) = -27
						else
							vxc_inp_matrix_gga(iter, 1) = log10(rho(flatten_idx, 1))
						end if
					end if
					!vxc_inp_matrix_gga(iter, 1) = log10(rho_3d(neigh_indices_i(i), neigh_indices_j(j), neigh_indices_k(k)))
					iter = iter + 1
				end do
			end do
		end do
		!print*, "vxc inp matrix "
		call gga_nn(vxc_inp_matrix_gga, fc1_W_gga, fc2_W_gga, fcout_W_gga, vxc_out_matrix)
		!print*, "vxc out matrix ", vxc_out_matrix
		if(order_file_exists) then
			vxc(inverse_mapper(ip), 1) = vxc_out_matrix(1, 1)
		else
  ...
  print*, "Vxc maximum ", maxval(vxc)
  print*, "Vxc minimum ", minval(vxc)
  print*, "Vxc mean ", sum(vxc)/float(2744)
  print*, "Density maximum ", maxval(rho)
  print*, "Density minimum ", minval(rho)
  print*, "Density mean ", sum(rho)/float(2744)
  print*, "Shape of rho and vxc", shape(rho), shape(vxc)
...
contains
  ...
  subroutine lda_process()
    ...
    elseif(ispin == SPIN_POLARIZED) then
      ...
      print*, "3#####"
      print*, shape(vxc), shape(dedd)
      call lalg_axpy(der%mesh%np, M_ONE, dedd(:, 1), vxc(:, 1))
      print*, "Maximum GRADS dedd vxc", M_ONE, sum(dedd(:, 1)), sum(vxc(:, 1))
    end if  
